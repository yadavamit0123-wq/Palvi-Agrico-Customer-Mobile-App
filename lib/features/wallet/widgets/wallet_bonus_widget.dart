  ...
  Widget build(BuildContext context) {
    return Padding(padding: const EdgeInsets.only(top : Dimensions.paddingSizeSmall, bottom: 50),
        child: Consumer<WalletController>(builder: (context, walletProvider, _) {
          final bonusLength = walletProvider.walletBonusModel?.bonusList?.length ?? 0;
          final isSingle = bonusLength == 1;
          ...
          Stack(clipBehavior: Clip.none, children: [
            ClipRRect(borderRadius: BorderRadius.circular(8),
              child: CarouselSlider.builder(options: CarouselOptions(
                  ...
                  autoPlay: !isSingle, enableInfiniteScroll: !isSingle, padEnds: false,
                  scrollPhysics: isSingle ? const NeverScrollableScrollPhysics() : null,
                onPageChanged: isSingle ? null : (index, reason) {walletProvider.setCurrentIndex(index);}),
